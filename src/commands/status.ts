...
export async function status(options: StatusOptions): Promise<void> {
  ...
  try {
    ...
    if (devcontainerExists) {
      ...
      const firewallScriptPath = path.join(devcontainerPath, 'init-firewall.sh');
      ...
      console.log(`  - Dockerfile: ${dockerfileExists ? chalk.green('✓') : chalk.red('✗')}`);
      console.log(`  - devcontainer.json: ${devcontainerJsonExists ? chalk.green('✓') : chalk.red('✗')}`);
      console.log(`  - init-firewall.sh: ${firewallScriptExists ? chalk.green('✓') : chalk.red('✗')}`);
    } else {
      console.log(chalk.red('✗ Devcontainer: Configuration not found (run "yolocon init")'));
    }
    ...
    if (!dockerRunning) {
      ...
      if (!claudeRunning) {
        ...
        console.log(chalk.yellow('• Run "yolocon open" to start VS Code and Claude Code'));
      } else {
